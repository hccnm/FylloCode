...
每条迁移 SHALL 是 `electron/main/infra/migrations/` 目录下的一个 TypeScript 文件，文件名格式为 `YYYYMMDD_NNN_<kebab-case-description>.ts`（如 `20260601_001_config-options-camel-case.ts`）。每个迁移文件 SHALL 导出一个 `migrate(ctx: MigrationContext): Promise<void>` 函数。
...
所有已注册迁移 SHALL 在 `electron/main/infra/migrations/index.ts` 中以有序数组形式静态声明，数组顺序即执行顺序，SHALL 与文件名字母序一致。

#### Scenario: 新增迁移脚本

- **WHEN** 开发者新增迁移文件 `electron/main/infra/migrations/20260601_003_foo.ts`
- **THEN** 该文件 SHALL 导出 `migrate(ctx: MigrationContext): Promise<void>`
- **AND** 开发者 SHALL 将其追加到 `electron/main/infra/migrations/index.ts` 的迁移数组末尾
