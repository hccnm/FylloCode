...
### Requirement: 内置 MCP server 在 ACP newSession 时注入

系统 SHALL 在 `services/chat/acp-session.ts` 创建新 ACP session 时，通过 `connection.newSession({ cwd, mcpServers })` 的 `mcpServers` 参数传入 `getBundledMcpServers({ projectPath })` 的返回值，而不是空数组。

#### Scenario: 新建 chat session 携带内置 MCP

- **WHEN** chat 首次发送消息，且该 `fylloSessionId` 不存在持久化的 `acpSessionId`
- **THEN** `connection.newSession` 被调用，且 `mcpServers` 至少包含一个 `name === "fyllo-specs"` 的 spec
- **AND** `cwd` 与 `projectPath` 保持原语义不变
