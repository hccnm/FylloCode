...
这个版本聚焦聊天体验收敛、版本可见性补齐与交互一致性优化。设置页 About 面板现在可以直接检查 GitHub 正式 Release 是否有新版本；聊天界面补上消息加载骨架屏，并优化了 Markdown 渲染开销；同时，Agent 在 probe 阶段返回的可用命令会被捕获并沿会话保存，为 Slash Commands 提供更稳定的数据基础。针对流式回复被取消或异常中断的场景，应用现在也会保留已生成的部分助手内容，避免消息直接丢失。

### 新增

- 设置页 About 面板新增 GitHub 正式 Release 检测能力，可检查是否有新版本，并直接跳转到对应的 Release 页面
- 聊天会话新增对 Agent 可用命令列表的捕获与保存，为草稿态和正式会话中的 Slash Commands 提供数据基础
